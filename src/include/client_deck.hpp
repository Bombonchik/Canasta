...
struct ClientDeck {
    ...
    size_t discardPileSize = 0;         // Number of cards in the discard pile
    size_t mainDeckSize = 0;            // Number of cards left in main deck
    ...
    ClientDeck(std::optional<Card> topCard, size_t discardSize, size_t deckSize, bool isFrozen);

    // --- Getters (Optional if members are public) ---
    const std::optional<Card>& getTopDiscardCard() const { return topDiscardCard; }
    size_t getDiscardPileSize() const { return discardPileSize; }
    size_t getMainDeckSize() const { return mainDeckSize; }
